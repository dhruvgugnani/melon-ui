...
import { motion, AnimatePresence, useMotionValue, useSpring, useTransform, HTMLMotionProps } from "framer-motion";
...
export function ZeroGravityShards({
  ...
  return (
    ...
    >
      ...
      {/* Floating Shards */}
      {shards.map((shard, i) => {
        // Individual parallax values for each shard based on initial position
        const parallaxX = useTransform(smoothMouseX, val => val * (shard.initialX / 500));
        const parallaxY = useTransform(smoothMouseY, val => val * (shard.initialY / 500));
      ...
      componentPath: "components/community/demos/ZeroGravityShards.tsx",
      scrollable: false,
...
export default function App() {
  ...
  export interface ChromaticSliceCardProps extends React.ComponentPropsWithoutRef<"div"> {
    title?: string;
    subtitle?: string;
    slices?: number;
    glowColor?: string;
