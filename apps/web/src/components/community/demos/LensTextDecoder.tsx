...
export interface LensTextDecoderProps extends React.ComponentPropsWithoutRef<"div"> {
  title?: string;
  clearText?: string;
  lensSize?: number;
  primaryColor?: string;
...
export function LensTextDecoder({
  title = "TOP SECRET",
  ...
  const lensX = useTransform(smoothX, (x) => x - lensSize / 2);
  const lensY = useTransform(smoothY, (y) => y - lensSize / 2);
