...
function FloatingOrb({ position, color, speed }: { position: [number, number, number]; color: string; speed: number }) {
  ...
  return (
    <mesh ref={meshRef} position={position}>
      <sphereGeometry args={[0.22 + Math.random() * 0.1, 32, 32]} />
...
const ORBS = [
  { position: [-1.4, 0, 0] as [number,number,number], color: "#ff5c71", speed: 1 },
  { position: [0, 0.6, -0.5] as [number,number,number], color: "#7fff5e", speed: 1.3 },
  { position: [1.4, -0.2, 0] as [number,number,number], color: "#e8d5b7", speed: 0.8 },
  { position: [-0.6, -0.7, 0.5] as [number,number,number], color: "#ff5c71", speed: 1.5 },
  { position: [0.8, 0.8, 0.3] as [number,number,number], color: "#7fff5e", speed: 0.9 },
];
